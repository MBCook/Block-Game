    ...
    while theEventQueue.isEmpty() == 0:
        ...
        if type == "Gamestate":
            ...
            if info == "Move Piece Down":
                ...
                xLoc = int(idealX / 8.0)
                yLoc = int(idealY / 8.0) + 1

                if (yLoc <= (boardHeight - 3)):
                    if (theBoard.pieceCanFit(xLoc, yLoc, thePiece)  <= 0):
                        idealY += 8.0
                    else:
                ...
                if (theBoard.pieceCanFit(xLoc, yLoc, tempPiece) <= 0):
                    thePiece = tempPiece
                else:
                    theEventQueue.addToQueueFront("Notification", "Can't Place")
