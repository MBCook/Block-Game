...
def loadTexture(name = "images_trans.png"):
	...
	textureSurface = pygame.image.load(os.path.join("images", name))
	...
	gluBuild2DMipmaps(GL_TEXTURE_2D, 4, 512, 512, GL_RGBA, GL_BYTE, dataString)
...
def drawNumber(x, y, z, w, num, digits):
	"""Draws a number using our texture, of "digits" digits in lengh,
		where each digit is "w" wide."""

	if (digits < 1) or (digits > 6):
		return	# Must be 1-6 digits
	
	for i in range(digits, 0, -1):
		digit = num / pow(10, i - 1)
		num = num - digit * pow(10, i - 1)
		drawDigit(x + (digits - i) * w, y, z, w, digit)

def drawDigit(x, y, z, w, digit):
	"""Draw the digit that is w units wide."""

	if (digit > 10) or (digit < 0):
		return	# Can't draw that

	sourceX = float((digit % 5) * 53)
	...
	width = w
	height = w
	sourceX = sourceX / 512.0
	sourceY = sourceY / 512.0
	texWidth = 52.0 / 512.0		# All numbers are 52 by 52 on our image
	texHeight = 52.0 / 512.0

	drawFromTexture(x, y, z, sourceX, sourceY, texWidth, texHeight, width, height)
...
def drawFromTexture(x, y, z, textureX, textureY, texWidth, texHeight,
	...
	glNormal3f(0.0, 1.0, 0.0)	# Things point up (into screen)!
...
def drawInfo(score, time, high):
	...
	# Now we draw the score

	z = z + height * 1.35
	scoreWidth = (40.0 * (2.0 / 3.0)) / 6.0 # 2/3rd the width of "Score", per digit
	scoreOffset = 40.0 * (1.0 / 3.0)

	drawNumber(x + scoreOffset, y, z, scoreWidth, score, 6)

	# Now we draw the high score

	highWidth = 353.0
	highHeight = 51.0

	texWidth = highWidth / imageWidth
	texHeight = highHeight / imageWidth

		# Height stays the same
	width = (highWidth / highHeight) * height

	textureX = 0.0
	textureY =	110.0/ imageWidth
	...
	drawNumber(x + scoreOffset, y, z, scoreWidth, high, 6)

	# Now we draw the time left	   

	timeWidth = 347.0
	timeHeight = 52.0

	texWidth = timeWidth / imageWidth
	texHeight = timeHeight / imageWidth

		# Height stays the same
	width = (timeWidth / timeHeight) * height

	textureX = 0.0
	textureY =	57.0/ imageWidth
	...
	scoreWidth = (40.0 * (2.0 / 3.0)) / 6.0 # 2/3rd the width of "Score", per digit
	scoreOffset = 40.0 * (1.0 / 3.0)
	scoreOffset = scoreOffset + (4.0 * scoreWidth)	# Should right justify

	drawNumber(x + scoreOffset, y, z, scoreWidth, time, 2)
